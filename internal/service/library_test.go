...
func TestLibraryPageLoadsCardMetadataWithScopedCounts(t *testing.T) {
	...
	empty := database.Movie.Create().SetCode("ABP-002").SaveX(ctx)
	...
	wantQueries := map[string]int{"*ent.SettingQuery": 1, "*ent.FileQuery": 1, "*ent.MovieQuery": 1, "*ent.TagQuery": 1}
	...
	for _, item := range page.Movies {
		if item.ID == film.ID {
			if item.Title != film.Title || item.Cover == nil || *item.Cover != "/cover" ||
				...
				!reflect.DeepEqual(item.Tags, []LibraryTag{{ID: tagA.ID, Name: tagA.Name}, {ID: tagB.ID, Name: tagB.Name}}) {
			...
			item.ScrapeStatus != movie.ScrapeStatusPending || item.Watched {
