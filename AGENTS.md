...
- Sonner 使用项目的 `--font-sans`。任务通知只展示当前阶段文字，随下载、扫描、核对、刮削和写回阶段更新，下方用一个紧凑的绿色 shadcn Progress 汇总阶段进度；扫描任务省略下载。不平铺所有阶段、不使用横向滚动或单阶段 `0/1` 展示。有实际进度的阶段按比例推进，未知总量阶段只按阶段切换推进，不伪造文件总量。查看、播放和关闭使用 shadcn 图标 Button 与 Tooltip，提示挂载在通知容器内；floating-nav 的播放按钮使用 `cursor-pointer`。
...
**播放**：媒体库卡片、详情页 floating-nav 与入库完成通知共用 Vidstack 无边框播放浮层。当前详情影片已入库时，在 floating-nav 的设置图标后显示竖向 shadcn Separator 和播放图标按钮，尺寸与其他导航按钮一致；预览图下方不再放播放按钮。浮层仅显示圆角视频画面，顶部展示影片标题，文件名只出现在文件选择菜单中，关闭按钮位于右侧；加载和报错时沿用相同布局。底部使用紧凑的半透明悬浮控制条，背景模糊，控件与背景一起淡入淡出。按番号读取当前媒体目录的全部关联视频，仅使用 115 HLS 转码，支持切换文件与可用清晰度；切换清晰度保留播放位置与暂停状态，切换文件从头开始，切换时保持播放器实例以保留全屏。选择菜单挂载在播放器内，全屏时仍可使用。关闭浮层立即卸载播放器、取消请求并释放播放会话。播放由用户主动发起，不受媒体图片的 NSFW 隐藏影响。

- 播放控件沿用 Vidstack 的状态与交互，图标统一使用 Lucide；按钮尺寸、圆角、悬停色和菜单使用项目主题变量，文件与清晰度选择复用现有 shadcn Select。按钮提示直接使用项目的 shadcn Tooltip，保留相同箭头、动画和默认间距，零等待延迟，以整个按钮范围为触发区域并居中显示在上方；普通模式挂载到 body，全屏模式挂载到播放器内，控制条隐藏时一并关闭提示。Vidstack 自带提示隐藏，不再单独模拟其样式或沿用左右对齐方式。
...
- 完整番号规范化不限制厂商前缀长度或流水号位数，支持 `SCUTE-1575-ITSUKI` 等名称后缀及连字符、下划线分隔的字母数字段，保留全部后缀与前导零。不能把文件名识别规则当作 JavDB 番号的白名单；完整字段中的 `C`、`CD1` 等也不自行截断。
- 文件名提取才处理字幕、清晰度和分集标记。SCUTE 文件名保留作品号后的名称段，普通影片文件名中的标题不作为番号后缀；浏览、详情关联影片与精确搜索匹配共用完整番号规范化规则。
...
- 前端 Vidstack 共用无边框浮层，115 HLS 转码、多文件与清晰度选择。
