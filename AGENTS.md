...
- 媒体库标题栏进度框与扫描按钮同为 `h-9`，紧凑模式的错误详情通过 Tooltip 展示，不能撑高进度框。进入媒体库或设置页时刷新任务快照；进度断开时额外刷新一次，并提供重建 SSE 的重连操作。彻底关闭或长时间无心跳的连接会重新建立。断线时扫描按钮显示“等待同步”，不能把旧的运行状态当成仍在处理而持续转圈。
...
**播放**：媒体库卡片与详情页共用 Vidstack 无边框播放浮层；详情页仅在影片已入库时于预览图下方显示播放按钮。浮层仅显示圆角视频画面，顶部展示影片标题，文件名只出现在文件选择菜单中，关闭按钮位于右侧；加载和报错时沿用相同布局。底部使用紧凑的半透明悬浮控制条，背景模糊，控件与背景一起淡入淡出。按番号读取当前媒体目录的全部关联视频，仅使用 115 HLS 转码，支持切换文件与可用清晰度；切换清晰度保留播放位置与暂停状态，切换文件从头开始，切换时保持播放器实例以保留全屏。选择菜单挂载在播放器内，全屏时仍可使用。关闭浮层立即卸载播放器、取消请求并释放播放会话。播放由用户主动发起，不受媒体图片的 NSFW 隐藏影响。
